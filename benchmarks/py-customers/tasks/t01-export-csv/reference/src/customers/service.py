...
from .exporters import customers_to_csv
...
class CustomerService:
    ...
    def list_customers(
        ...
        """Return customers sorted by id, optionally filtered."""
        return self._repository.list_all(status=status, search=search)
    ...
    def export_csv(self, *, status: str | None = None, search: str | None = None) -> str:
        """Return the (optionally filtered) customer list rendered as CSV."""
        return customers_to_csv(self.list_customers(status=status, search=search))
