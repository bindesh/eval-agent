...
class CustomerService:
    ...
    def import_customers(self, rows: Iterable[Mapping[str, str]]) -> list[Customer]:
        ...
        for row in rows:
            customer = Customer(
                id=require_non_empty(row.get("id"), "id"),
                name=require_non_empty(row.get("name"), "name"),
                email=require_email(row.get("email"), "email"),
                status=require_choice(row.get("status", "active"), "status", VALID_STATUSES),
            )
